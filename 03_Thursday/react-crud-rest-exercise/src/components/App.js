...
function App({ apiFacade }) {
  ...
  const storeAddEditPerson = (person) => {
    ...
    apiFacade.addEditPerson(person, (data) => setPersons([...data]));
    setPersonToAddEdit({ ...emptyPerson });
    //console.log("emptyPerson", emptyPerson);
    // console.log("editPerson", personToAddEdit);
  };

  const deletePerson = (id) => {
    //Call this from the AllPerson control with the id for the person to delete
    apiFacade.deletePerson(id, (data) => setPersons([...data]));
